...
export async function POST(req: Request) {
  ...
  try {
    const data = await httpClient.post<unknown>('/gam/cancel-registration', body, { token: proxyToken })
    return ok(data)
  } catch (error) {
    if (error instanceof HttpError) return fail(error.status, 'BACKEND_ERROR', 'Error en backend GAM', error.body)
    return fail(500, 'INTERNAL_ERROR', 'Error inesperado en cancel-registration')
  }
}
