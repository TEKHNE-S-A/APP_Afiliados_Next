# Test del servicio REC_SOLICITUDES_APP
# Requiere usuario autenticado para obtener nuusuid

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "TEST: REC_SOLICITUDES_APP (POST /sia/solicitudes)" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# Paso 2: Llamar al servicio REC_SOLICITUDES_APP
Write-Host "`nPaso 2: Llamando a REC_SOLICITUDES_APP..." -ForegroundColor Yellow
...
Write-Host "`nNota: AUSolIdExt se obtiene automáticamente del usuario autenticado (nuusuid: $nuusuid)" -ForegroundColor Cyan

try {
    $response = Invoke-RestMethod -Uri "http://localhost:3000/sia/solicitudes" `
        -Method Post `
        -Headers $headers `
        -Body $body
