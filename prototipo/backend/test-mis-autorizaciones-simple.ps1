# Test GET /mis-autorizaciones - Endpoint con sincronizacion SOAP + BD
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host "  TEST: GET /mis-autorizaciones" -ForegroundColor Cyan
Write-Host "  Descripcion: SOAP + BD Sync" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($response.autorizaciones -and $response.autorizaciones.Count -gt 0) {
        ...
        foreach ($auth in $response.autorizaciones) {
            ...
            Write-Host "   Descripcion: $($auth.descripcion)" -ForegroundColor White
            ...
            Write-Host "   Num Autorizacion: $($auth.autorizacion_numero)" -ForegroundColor White
            Write-Host "   Prestacion ID: $($auth.tipo_prestacion_id)" -ForegroundColor White
            Write-Host ""
            $contador++
        }
    } else {
        Write-Host "No se encontraron autorizaciones" -ForegroundColor Yellow
    }
